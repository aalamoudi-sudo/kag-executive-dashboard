(() => {
  ...
  function bind(){
    ...
    // ============================================================
    // Event delegation لأزرار مركز التقارير — يعمل بعد كل re-render
    // ============================================================
    async function doGenerateReport(rtype, rname, btnEl){
      ...
      try{
        ...
        a.download = `KAGA-${(rname||rtype).replace(/\s+/g,"-")}-${dateStr}.pptx`;
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
        URL.revokeObjectURL(url);
        toast(`✅ تم تجهيز ${rname||rtype} بنجاح — جارٍ التنزيل`);
      }catch(e){
        toast(`❌ ${e.message}`);
      }finally{
    ...
    document.body.addEventListener("click", async e=>{
      ...
      if(tileBtn){
        e.stopPropagation();
        await doGenerateReport(tileBtn.dataset.rtype, tileBtn.dataset.rname, tileBtn);
        return;
      }
      // زر التقرير الشامل
      ...
      if(compBtn){
        e.stopPropagation();
        compBtn.disabled=true; compBtn.textContent="جارٍ التوليد...";
        try{
          const res=await fetch("/api/report",{method:"POST",credentials:"include",headers:{"Content-Type":"application/json"},body:JSON.stringify({type:"comprehensive"})});
          if(!res.ok) throw new Error("تعذر توليد التقرير الشامل");
          const blob=await res.blob(); const url=URL.createObjectURL(blob);
          const a=document.createElement("a"); a.href=url;
          a.download="KAG-Report-Comprehensive-"+new Date().toISOString().slice(0,10)+".pptx";
          document.body.appendChild(a); a.click(); document.body.removeChild(a);
          URL.revokeObjectURL(url);
          toast("✅ تم توليد التقرير الشامل — جارٍ التنزيل");
        }catch(e){ toast(`❌ ${e.message}`); }
        finally{ compBtn.disabled=false; compBtn.textContent="توليد تقرير شامل"; }
        return;
      }
    });
  }
